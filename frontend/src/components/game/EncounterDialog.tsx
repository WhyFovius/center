...
function buildChecklistKeys(step: ScenarioStep): string[] {
  ...
  if (family.includes('wifi') || family.includes('кафе')) return ['clWifi1', 'clWifi2', 'clWifi3', 'clWifi4'];
  if (family.includes('sms') || family.includes('qr') || family.includes('смартф')) return ['clMobile1', 'clMobile2', 'clMobile3', 'clMobile4'];
  if (family.includes('звонок') || family.includes('соц') || family.includes('otp')) return ['clSocial1', 'clSocial2', 'clSocial3', 'clSocial4'];
...
export function EncounterDialog({ step, onClose }: { step: ScenarioStep; onClose: () => void }) {
  ...
  const handleSubmit = (optionId: number) => {
    ...
    sfx.click();
    submit(step.id, optionId, getHints(step.id));
  };
    ...
    >
      ...
      >
        <div className="relative border-b border-border px-6 py-5 bg-[linear-gradient(180deg,rgba(45,139,77,0.08),rgba(45,139,77,0.02))]">
          <button onClick={onClose} className="absolute right-4 top-4 rounded-full p-2 text-text-muted hover:bg-white/80 transition-colors">
            <X className="w-4 h-4" />
          </button>
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
            <motion.section initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }}
              ...
              <div className="mt-4 space-y-3">
                {checklistKeys.map((itemKey, index) => (
                  <motion.div key={itemKey} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
            ...
            <motion.section initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25, delay: 0.08 }}
              ...
              <p className="mt-3 text-sm leading-relaxed text-text-secondary">{T('encounterReadinessDesc')}</p>
              <div className="mt-5 rounded-2xl border border-border bg-white p-4">
                <div className="flex items-start gap-2">
                  <ShieldCheck className="w-5 h-5 text-primary mt-0.5 shrink-0" />
                  <p className="text-sm text-text-secondary">{T('encounterPrinciple')}</p>
                </div>
              </div>
            </motion.section>
          </div>

          <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3, delay: 0.18 }}
            className="mt-6 rounded-[24px] border border-border bg-white p-5">
            <p className="text-base md:text-lg font-semibold text-text">{question}</p>

            <div className="mt-4 grid grid-cols-1 gap-3">
              {answers.map((answer, index) => (
                <motion.button key={answer.id}
                  initial={{ opacity: 0, y: 8 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ opacity: { duration: 0.25, delay: 0.24 + index * 0.08 }, y: { duration: 0.25, delay: 0.24 + index * 0.08 } }}
                  whileHover={{ scale: 1.01, y: -1 }}
                  whileTap={{ scale: 0.995 }}
                  onClick={() => handleSubmit(answer.optionId)}
                  disabled={submitting}
                  className="w-full text-left rounded-2xl border border-border bg-bg-secondary/45 px-4 py-4 shadow-sm transition-all duration-200 hover:shadow-[0_14px_28px_rgba(0,0,0,0.08)] text-text"
                >
                  <span className="text-sm md:text-base font-semibold">{answer.label}</span>
                  {answer.details && <span className="mt-1 block text-xs text-current/70">{answer.details}</span>}
                </motion.button>
              ))}
            </div>
